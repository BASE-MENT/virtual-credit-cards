...
const NumberInput = require("./number");
const ExpirationInput = require("./expiration");
const CvcInput = require("./cvc");
  ...
  return h("form", [
        ...
        NumberInput.render(
          ...
          renderStyle(
            NumberInput.validate(
              state.number,
              Object.keys(state.types).filter(function (name) {
    ...
    h(".row", [
      h("label.small-6.columns", [
        "Expiration",
        ExpirationInput.render(
          state.expiration,
          renderStyle(ExpirationInput.validate(state.expiration))
        ),
      ]),
      h("label.small-6.columns", [
        "CVC",
        CvcInput.render(
          state.cvc,
          renderStyle(
            CvcInput.validate(state.cvc, card.type(state.number.value))
          )
        ),
      ]),
    ]),
    ...
    h(".row.small-12.columns", [
      h(
        ".callout",
        json.render({
          types: Object.keys(state.types).filter(function (type) {
          ...
          number: {
            ...
            valid: NumberInput.validate(
              state.number,
              Object.keys(state.types).filter(function (type) {
                return state.types[type];
              })
            ),
          },
...
function renderTypeControls(state) {
  return h("fieldset.fieldset", [
    ...
    Object.keys(state.types).map(function (type, index) {
      const id = "card-type-" + index;
      const checked = state.types[type];
...
function Types(types) {
  const data = types.reduce(function (acc, type) {
