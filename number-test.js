...
test("expiration", function (t) {
  t.test("state to dom", function (t) {
    ...
    function assertFormatted(input, output) {
      const state = NumberInput({
        value: input,
      });
      t.equal(
        createElement(NumberInput.render(state())).value,
        output,
        `format: ${JSON.stringify(input)} -> "${output}"`
      );
    }
  });
  ...
  t.test("dom to state", function (t) {
    t.plan(7);
    const render = thermometer.createComponent(NumberInput);
    ...
    render(function (state, element, done) {
      ...
      t.equal(
        state.value(),
        "5115000000001234",
        "prevents overtyping into value"
      );
    });
  });
