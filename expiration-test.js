...
test("expiration", function (t) {
  t.test("state to dom", function (t) {
    ...
    function assertFormatted(input, output) {
      const state = Expiration({
        raw: typeof input === "string" && input,
        value: typeof input === "object" && input,
      });
      t.equal(
        createElement(Expiration.render(state())).value,
        output,
  ...
  t.test("dom to state", function (t) {
    t.plan(9);
    const render = thermometer.createComponent(Expiration);
      ...
      raf(function () {
        ...
        raf(function () {
          t.equal(
            element.value,
            "01 / 1500",
            "does not truncate non-2000 dates"
          );
        });
      });
    });
